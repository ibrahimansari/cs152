...
class SL1Parser extends JavaTokenParsers {    
  ...
  def term: Parser[Expr] = (cons ~ rep(("*" | "/" ) ~ cons)) ^^ {
      case a ~ lst =>  (a /: lst) { 
        case (x, "*" ~ y) => Operator(x, y, _ * _)
        case (x, "/" ~ y) => Operator(x, y, _ / _)
      }
  ...
  def cons: Parser[Expr] = factor ~ opt("::" ~ (cons | ident)) ^^ {
    case expr ~ None => expr
    case expr ~ Some("::" ~ (listVar: String)) => Cons(expr, Variable(listVar))
    case expr ~ Some("::" ~ (list: Expr)) => Cons(expr, list)
  }
...
object SL1 extends App {
  ...
  val initList = List(("Nil", List()), ("head", ListOp(l => l.head)), ("tail", ListOp(l => l.tail)), ("isEmpty", ListOp(l => if (l.isEmpty) 1 else 0)))

  val parser = new SL1Parser
  val parseResult = parser.parseAll(parser.block, new InputStreamReader(System.in))
  parseResult match {
    case parser.Success(result, next) => println(evalBlock(result, initList))
